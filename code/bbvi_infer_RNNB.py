...
class GenerativeSLDS(nn.Module):
    ...
    def log_transition_likelihoods(self, zs, ys, temperature, mask=None):
        ...
        logits = stabilize_logits(logits, name="transition_logits")
...
def predict_k_step_more(k_max, model, variational_z, ys, 
    ...
    """RNN-transition k-step prediction helper.

    This minimally fixes the original RNN file's helper, which referenced the
    linear-sticky transition parameters Rs/r/gamma.
    """
    device = next(model.parameters()).device
    ys_tensor = torch.as_tensor(ys, dtype=torch.float32, device=device)  # [B,T,N] or [T,N]
    if ys_tensor.dim() == 2:
        ys_tensor = ys_tensor.unsqueeze(0)
    B, T, N = ys_tensor.shape
    temp = temperature if torch.is_tensor(temperature) else torch.tensor(temperature, device=device)

    y_preds = torch.zeros((B, T, k_max, N), device=device)
    y_input = ys_tensor.unsqueeze(0).expand(n_trajectory, -1, -1, -1).clone()  # [M,B,T,N]
    z_preds = variational_z.sample_q_z(y_input, temp)  # [M,B,T,K]

    for k in range(1, k_max + 1):
        projected = torch.einsum('mbtn,dn->mbtd', y_input[:, :, :-1], model.F)  # [M,B,T-1,D]
        h, _ = model.trans_rnn(projected.reshape(-1, projected.shape[2], projected.shape[3]))
        h = h.view(n_trajectory, B, -1, h.shape[-1])  # [M,B,T-1,H]
        logits_z = model.trans_out(h)                 # [M,B,T-1,K]
        logits_z = stabilize_logits(logits_z, name="predict_transition_logits")

        logits_exp = logits_z.unsqueeze(0).expand(n_samples, -1, -1, -1, -1)
        z_full = dist.RelaxedOneHotCategorical(temp, logits=logits_exp).sample()
