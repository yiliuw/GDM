...
def fit_bbvi_schedule(model, ys, xs0=None, num_iters=100, learning=True, n_samples=10, 
                     base_lr=1e-2, warmup_iters=30, tau_max=0.1, temp_annealing=True):
    device = ys.device
    ...
    warmup = LinearLR(optimizer, 
                     start_factor=0.1,  # Starts at 0.1 * base_lr (0.001)
    ...
    decay = LinearLR(optimizer,start_factor=1.0, end_factor=0.5,total_iters=8*warmup_iters)
    hold = ConstantLR(optimizer,factor=0.5,total_iters=float('inf'))
    
    scheduler = SequentialLR(optimizer, 
                           schedulers=[warmup, decay, hold],
                           milestones=[warmup_iters, 8*warmup_iters])
    ...
    for itr in pbar:
        ...
        def ELBO_objective(params, itr, zs, xs, tau):
            ...
            return (log_pz + log_px + log_py - log_qx - log_qz).mean()+ 100* state_entropy
        ...
        if len(elbo_history) > convergence_window:
            ...
            if elbo_improvement < 1e-2:  ## avg ELBO not increasing
                print(f"Early stopping at iteration {itr}.")
                break
            # --- TEMP ANNEALING ---
            if temp_annealing and itr > warmup_iters:   ## temperature annealing as LR
                tau = torch.max(tau * torch.exp(-10*torch.tensor(prev_lr - current_lr, device=device)), 
                          torch.tensor(0.5, device=device)) 
            too_crisp = (zs < 1e-8).any() or (zs > 1 - 1e-8).any()
            if too_crisp:
                tau = torch.tensor(tau_max, device=device) 
...
class GenerativeSLDS(nn.Module):
    ...
    def log_transition_likelihoods(self, zs, xs, temperature):
        log_z1 = F.log_softmax(self.logits_z1 / temperature, dim=-1)
        log_prior_z1 = (zs[:, 0] * log_z1).sum(-1)
        
        # Transition Probabilities
        log_Ps = torch.einsum('ntd,kd->ntk', xs[:, :-1], self.Rs) + self.r
        log_softmax = F.log_softmax(log_Ps / temperature, dim=-1)
        log_probs = (zs[:, 1:] * log_softmax).sum(-1).sum(1)
