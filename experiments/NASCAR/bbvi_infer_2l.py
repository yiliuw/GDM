...
def predict_k_steps_full(model, variational_z, ys, k=2, n_trajectory=10, temperature=0.99):
    ...
    z_paths = variational_z.sample_q_z(
        ys_tensor.unsqueeze(0).expand(n_trajectory, -1, -1), temperature)  # [n_trajectory, T, K]
    ...
    for step in range(1, k):
        ...
        # Build children z [n_paths, n_trajectory, T, K]
        ...
        # for each flattened child, smooth with that child's own ys 
        y_pred_list = []
        ...
        # Stack predicted ys for all children
